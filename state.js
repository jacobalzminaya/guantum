...
function loadConfig() {
    ...
    if (typeof refreshVisualButtons === 'function') {
        // Ejecución inmediata para intentar capturar el DOM rápido
        refreshVisualButtons(); 
        
        // Ejecución de seguridad: Si el DOM estaba "ocupado", esto lo fuerza a los 50ms
        // 200ms era mucho tiempo y permitía que el usuario viera el error.
        setTimeout(refreshVisualButtons, 50); 
    }
}
...
/**
 * Sincroniza toda la interfaz gráfica con las variables de estado internas.
 * Fusiona filtros, botones de nivel r1-r3 y el visor central de mensajes.
 */
function refreshVisualButtons() {
    // 1. ACTUALIZAR ESTADO DE BOTONES DE FILTRO (Trend, Flex, Neural, etc.)
    ...
    // 2. CONFIGURACIÓN DE NIVELES (DICCIONARIO DE ESTILOS)
    const riskBtn = document.getElementById('risk-cycle-btn');
    const opDetails = document.getElementById('op-details');
    
    // Se añade "boost" para definir el riesgo extra que muestra el Flex en cada nivel
    const config = {
    ...
    // Validación de seguridad para el nivel actual
    if (typeof riskLevel === 'undefined') riskLevel = 3;
    const current = config[riskLevel] || config[3];

    // 3. ACTUALIZAR BOTONES INDIVIDUALES (r1, r2, r3) SI EXISTEN
    for (let i = 1; i <= 3; i++) {
        const rBtn = document.getElementById(`r${i}`);
        if (rBtn) {
            rBtn.classList.toggle('active', riskLevel === i);
        }
    }

    // 4. APLICAR IDENTIDAD VISUAL AL CICLADOR (BOTÓN CENTRAL)
    if (riskBtn) {
    ...
    // 5. ACTUALIZAR VISOR DE MENSAJES CON LÓGICA FLEX ACTIVA
    if (opDetails) {
        if (typeof flexMode !== 'undefined' && flexMode) {
            // MODO FLEX: Inyectamos el texto compuesto y el parpadeo de alerta
            opDetails.innerHTML = `
            ...
            // MODO NORMAL: Solo el detalle del nivel
