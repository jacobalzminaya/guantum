...
function updateAnalyticUI(noise, power, ms, recent, majorTrend) {
    ...
    // --- [NUEVO] INTEGRACIÓN QUIRÚRGICA: DETECCIÓN SMC/ICT 2026 ---
    ...
    // --- LÓGICA DEL ASESOR DE IA MODIFICADA ---
    ...
    // Prioridad Máxima: Modelos Institucionales Detectados
    else if (activeSetup) {
    ...
    // Aplicar Mensaje de Estado (SMC Adaptado)
    ...
    if (iaLogic && iaStake) {
        ...
        if (totalOps < 10) {
            ...
            iaLogic.innerHTML = `NEURAL: <span style="color:${colorIA}">${probPct}%</span> | SMC: <span style="color:gold;">${superScore.toFixed(1)}</span>`;
            
            // Sugerencia basada en confluencia total (SMC + Neural)
